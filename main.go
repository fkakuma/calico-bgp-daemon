...
func (s *Server) AddNeighbor(n *svbgpconfig.Neighbor) error {
	...
	var typ svbgpconfig.AfiSafiType
	if err == nil {
		if ipAddr.IP.To4() == nil {
			typ = svbgpconfig.AFI_SAFI_TYPE_IPV6_UNICAST
		} else {
			typ = svbgpconfig.AFI_SAFI_TYPE_IPV4_UNICAST
		}
	}

	n.AfiSafis = []svbgpconfig.AfiSafi{
		svbgpconfig.AfiSafi{
			Config: svbgpconfig.AfiSafiConfig{
				AfiSafiName: typ,
				Enabled:     true,
			},
			MpGracefulRestart: svbgpconfig.MpGracefulRestart{
				Config: svbgpconfig.MpGracefulRestartConfig{
					Enabled: true,
				},
			},
			State: svbgpconfig.AfiSafiState{
				AfiSafiName: typ,
			},
		},
	}
